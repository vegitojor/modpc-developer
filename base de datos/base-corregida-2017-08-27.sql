...
CREATE TABLE IF NOT EXISTS `modpc`.`Ficha_tecnica` (
  ...
  `nombre_ficha` VARCHAR(45) NULL,
  `cadena_campos` VARCHAR(500) NULL,
...
CREATE TABLE IF NOT EXISTS `modpc`.`categoria` (
  ...
  `Ficha_tecnica_id_Ficha_tecnica` INT NULL,
  PRIMARY KEY (`id_categoria`))
ENGINE = InnoDB;

CREATE INDEX `fk_categoria_Ficha_tecnica1_idx` ON `modpc`.`categoria` (`Ficha_tecnica_id_Ficha_tecnica` ASC);
...
CREATE TABLE IF NOT EXISTS `modpc`.`valoracion_producto` (
  ...
  PRIMARY KEY (`producto_id_producto`, `cliente_id`))
ENGINE = InnoDB;

CREATE INDEX `fk_valoracion_producto_cliente1_idx` ON `modpc`.`valoracion_producto` (`cliente_id` ASC);
